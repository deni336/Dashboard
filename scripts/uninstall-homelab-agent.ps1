...
$InstallRoot = [System.IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA "Kasugai\homelab-agent"))
...
if ($RemovePolicy) {
    $PolicyPath = Join-Path $InstallRoot "config.json"
...
Write-Host "Kasugai homelab agent removed."
if (-not $RemoveCredentials) {
    Write-Host "The separate DPAPI-protected pairing was preserved. Use -RemoveCredentials to delete it."
}
if (-not $RemovePolicy) {
    Write-Host "The local opt-in policy was preserved. Use -RemovePolicy to delete it."
}
